...
>) {
  ...
  return (
    <span
      className={cn(
        "relative inline-flex h-10 items-center gap-1 rounded-sm px-1.5 focus-within:ring-2 focus-within:ring-inset focus-within:ring-primary",
...
export function TableEnumField<T extends string>({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        onClick={(e) => e.stopPropagation()}
        className="inline-flex h-10 cursor-pointer items-center rounded-sm px-1.5 text-left hover:bg-muted/40 focus:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:opacity-50"
        disabled={pending}
      >
        {value !== null ? (
          renderValue(value)
