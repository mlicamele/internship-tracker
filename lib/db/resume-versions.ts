...
export async function getMainResumeVersion(
  supabase: SupabaseClient,
  userId: string
...
// Single UPDATE via RPC so the partial unique index (user_id) WHERE is_main
// evaluates at statement end rather than per-row. See migrations 0015 + 0020.
export async function setMainResumeVersion(
  supabase: SupabaseClient,
  userId: string,
  id: string
): Promise<void> {
  const { error } = await supabase.rpc("set_main_resume_version", {
